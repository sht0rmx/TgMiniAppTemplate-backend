...
async def generate_recovery(request: Request):
    ...
    try:
        ...
        try:
            await bot_websocket.send_json(
                message={"action": "send:recoverycode", "data": {"code": code}}
            )
        except WSNotFound:
            return JSONResponse(content={"code": code, "detail": "cant send code to bot"}, status_code=200)
